...
class Lupi_Resource_Odm extends Zend_Application_Resource_ResourceAbstract {
    public function init() {
        /* 
         * Get a Mongo object with the corresponding DSN via getMongoObject.
         * The object is passed to the create function of the DocumentManager.
         * When nothing is passed, a connection to the local mongodb instance is build.
         */
        $mongo = $this->getMongoObject();
    ...
    /**
     * Creates a DSN (Data Source Name) and creates on its basis a new Mongo object.
     ...
     */
    public function getMongoObject($hostname='127.0.0.1', $port='27017', $username='', $password = '', $database='') {
        // local host
        ...
        // remote host
            ...
            $dsn = sprintf('mongodb://%s:%s@%s:%s/%s', $username, $password, $hostname, $port, $database);
        }
